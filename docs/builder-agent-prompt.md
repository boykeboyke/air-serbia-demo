...
2. **`get_flight_status`** - GET `[BASE_URL]/api/flight-status/{flight_number}`. **This is a live
   ...
   and times.
3. **`change_booking`** - POST `[BASE_URL]/api/booking/change` with `{pnr, new_date, new_flight_number, notes}`.
4. **`add_baggage`** - POST `[BASE_URL]/api/baggage/add` with `{pnr, extra_bags}`.
5. **`check_in`** - POST `[BASE_URL]/api/checkin` with `{pnr, seat}`.
6. Built-in **`transfer_to_agent`** and **`goodbye_and_hang_up`**.
